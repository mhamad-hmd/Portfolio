...
function reducer(state: any, action: any) {
    switch (action.type) {
        ...
        case "gestureMove": {
            return {
                ...state,
                transform: action.currentPosition - state.initialPosition
            }
        }
...
export default function DisplayProjects({ className }: { className: string }) {
    ...
    const { transform, initialPosition, moving } = state;
    ...
    return (
        <div ref={projectsRef} className={`workWrapper   transition-all duration-500 ease-in-out w-full   h-full flex section  justify-center items-center  md:mx-5  flex-col absolute top-0 ${className}`}>
            ...
            <div ref={displayRef} className={`  relative justify-between items-center flex gap-12 max-w-full  w-full  `}   >

                <div className="myWorkNav flex justify-center items-center whitespace-nowrap h-full  ">
                    <ul className="categoriesWrapper mainFont / flex flex-col gap-8 justify-center p-4  /  h-full " onClick={() => setMainProject("")}>
                        <li className={`catigoryTitle mainFont scndryColor text-3xl `} onClick={() => handleNavClick('All')}>All</li>
                        <li className={`catigoryTitle mainFont scndryColor text-3xl `} onClick={() => handleNavClick("Shopify")}>Shopify</li>
                        <li className={`catigoryTitle mainFont scndryColor text-3xl `} onClick={() => handleNavClick('Full-Stack')}>Full-Stack</li>
                    </ul>
                </div>
                                ...
                                if (mainProject == "") {
                                    if (category == "Shopify") {
                                        ...
                                        return (
                                            ...
                                            <div className={`projectCardWrapper h-96 max-w-lg    flex-shrink-0 ${category ? 'viewWork' : 'offWorkLeft'} cursor-pointer `} onClick={() => setMainProject(project.title)}>
                                        ...
                                        return (
                                            <div unselectable="on" className={`projectCardWrapper pointer-events-none h-96 max-w-lg   flex-shrink-0  ${category ? 'viewWork' : 'offWorkLeft'} cursor-pointer`} onClick={() => setMainProject(project.title)}>
                                                <ProjectCard mainProject={mainProject} scrollSpeed={0.2} myParallaxView={myElementIsVisible} even={i % 2 == 0 ? true : false} index={project.index} title={project.title} typeOfwork={project.typeOfwork} password={project.password} imgPath={project.imgPath} projectUrl={project.url} />
                                            </div>
                                        )
                                    }
